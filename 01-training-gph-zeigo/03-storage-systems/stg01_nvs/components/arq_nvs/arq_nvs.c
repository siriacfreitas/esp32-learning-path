...
/**
 * @brief Inicializa a memória flash e prepara o handler do driver.
 * 
 ...
 */
esp_err_t nvs_init(arq_nvs_handler_t *handle, arq_nvs_config_t *config) {
    ...
    /** @step 2: Configuração do Handler e criação do Mutex para segurança RTOS. */
    ...
    ESP_LOGI(TAG, "Driver pronto. Namespace: %s, Key: %s", config->namesp, config->key);
    return ESP_OK;
}
 ...
 * 
 * @param handle Handler inicializado.
 * @return esp_err_t ESP_OK se gravado e comitado com sucesso.
 */
esp_err_t nvs_write(arq_nvs_handler_t *handle) {
...
/**
 * @brief Lê o valor da Flash e armazena na variável vinculada ao handler.
 * 
 * @param handle Handler inicializado.
 * @return esp_err_t ESP_OK ou ESP_ERR_NVS_NOT_FOUND se a chave for nova.
 */
esp_err_t nvs_read(arq_nvs_handler_t *handle) {
    if (!handle || !handle->nvs_config.data) return ESP_ERR_INVALID_ARG;
 ...
 * 
 * @param handle Handler a ser desativado.
 * @return esp_err_t ESP_OK em sucesso.
 */
esp_err_t nvs_deinit(arq_nvs_handler_t *handle) {
